...
public class BaseScreen {
	...
	public void instantiateBrowser(String selectedBrowser,String selectedPlatform,
		...
		if (selectedBrowser.equalsIgnoreCase(Constants.BROWSER_CHROME)) {
		...
		else if (selectedBrowser.equalsIgnoreCase(Constants.ANDROID_WEBDRIVER)) {
			try{
			log.info("-------------***LAUNCHING ANDROID_WEBDRIVER***--------------");
			capabilities = new DesiredCapabilities();
			capabilities.setBrowserName("android");
		    capabilities.setJavascriptEnabled(true);
			System.out.println("-----------Checking in ANDROID_WEBDRIVER-------");
			}
			catch (Exception e)
			{
				e.printStackTrace();
			...
			throw new ScreenException(
					"------Only FireFox,InternetExplore Chrome html ANDROID_WEBDRIVER ,iPhoneWebDriver works-----------");
		}
